...
public class DeepseekHttpauto {
    ...
    private static String getfetch_pageJsonStr() {
        String pageurl = "https://chat.deepseek.com/api/v0/chat_session/fetch_page";
        HttpRequest request = HttpRequest.post(pageurl);
        setHeader(request);
        HttpResponse response = request.execute();
        log.info("响应数据: {}", response.body());
    ...
    private static void setHeader(HttpRequest request) {
        request.header("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:139.0) Gecko/20100101 Firefox/139.0")
                .header("Accept", "*/*")
